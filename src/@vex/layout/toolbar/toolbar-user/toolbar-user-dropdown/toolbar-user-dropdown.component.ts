...
export class ToolbarUserDropdownComponent implements OnInit {
  ...
  ngOnInit() {
    const rol = this.tokenService.getUsuarioJWT().roles[0].nombre;

    switch (rol) {
      case 'admin':
        this.rol = 'Administración';
        break;
      case 'venta':
        this.rol = 'Ventas';
        break;
    ...
    this.authService.logout({token})
      .subscribe((response: any) => {
        if (response.status === 200) {
          ...
          this.mostrarSnackbar('Error al cerrar sesión', 'red-snackbar');
        }
      }, err => {
        this.mostrarSnackbar('Error al cerrar sesión', 'red-snackbar');
      }, () => {
        this.mostrarSnackbar('Cierre de sesión exitoso', 'green-snackbar');
      });
  }
